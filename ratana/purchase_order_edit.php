...
<div class="row" style="background-color: #f2f2f2">
	<div class="col-sm-8">
		<h1 id="h2">Edit Purchase Order</h1>
	</div>
	<div class="col-sm-4">
	...
	<div class="">
		...
		<div class="row">
				...
				<select name="supplier" class="form-control" id="supplier_select">
					...
					while($row = mysqli_fetch_array($retval)) {
						...
						if($po_data["supplier_id"] == $id){
							echo "<option selected value='$id'>{$row['supplier_name']}</option>";
			...
			<div class="col-sm-6">
				...
				<textarea name="shipping" class="form-control" id="shipping" rows="3"><?php echo $supplier_address; ?></textarea>
			</div>
		</div>
		<br><br>
		<div class="row">
			...
			if($res && $res->num_rows > 0){
				while($row = $res->fetch_assoc()){
					...
					echo "<div class='form-row'>
							...
							<div class='form-group col-md-1'>
								<input type='number' min='1' class='form-control quantity' value='$quantity'>
							</div>
							<div class='form-group col-md-2'>
	...
	<div class="row">
		...
		<div class="col-sm-5">
			<ul class="nav nav-pills" style="float:right;">
				...
				<li role="presentation"> <a id="id-po-save" class="btn btn-warning">Update</a></li> &emsp;
			</ul>
		</div>
	</div>
</div>
			...
			if(supplier_id && supplier_id != "null"){
				...
				$("#items").children().each(function(index, form_row){
					...
					if(product_id && product_id != "null"){
						var unit_cost = $(form_row).find(".unit_cost").val();
						var quantity = $(form_row).find(".quantity").val();
						var pod_id = $(form_row).find(".po-class-pod-id").val();
						var item = {"pod_id":pod_id, "product_id": product_id, "unit_cost": unit_cost, "quantity": quantity};
						products.push(item);
					}
				});
